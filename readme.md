<p align="center">
    <img src="https://github.com/rose-pine/rose-pine-theme/raw/main/assets/icon.png" width="80" />
    <h2 align="center">Rosé Pine for Ox</h2>
</p>

<p align="center">All natural pine, faux fur and a bit of soho vibes for the classy minimalist</p>

<p align="center">
    <a href="https://github.com/rose-pine/rose-pine-theme">
        <img src="https://img.shields.io/badge/community-rosé%20pine-26233a?labelColor=191724&logo=data:image/svg+xml;base64,&style=for-the-badge" />
    </a>
</p>

## Usage

- Move one or more of `rose-pine{-dawn,-moon}.lua` to `~/.config/ox`.

```sh
cp -r ./dist/ ~/.config/ox
```

- Load one of the the plugins in `.oxrc`.

```lua
-- ~/.oxrc

load_plugin("rose-pine.lua")
```

## Gallery

![Ox with Rosé Pine](./gallery/rose-pine.png)

![Ox with Rosé Pine Moon](./gallery/rose-pine-moon.png)

![Ox with Rosé Pine Dawn](./gallery/rose-pine-dawn.png)

## Thanks to

- [SLUCHABLUB](https://github.com/SLUCHABLUB)

## Contributing

Modify `template.lua` using Rosé Pine variables, then build the variants:

```sh
npx @rose-pine/build@latest -t template.lua
```
